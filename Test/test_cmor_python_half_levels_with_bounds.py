...
class TestCase(base_test_cmor_python.BaseCmorTest):
    ...
    def setUp(self):
        ...
        self.specs = {"CLOUD": {"convert": [2.0, 0.], "units": "%", "entry": "cl", "positive": ""},
    ...
    def read_time(self, index, refyear=2015., monthdays=30., yeardays=360.):
    ...
    def read_3d_input_files(self, index, varname, shape):

        print "3d shape",shape
        field = numpy.zeros(shape, dtype=numpy.float32)
        factor, offset = self.specs[varname]["convert"]
    ...
    def read_2d_input_files(self, index, varname, shape):
        sh = list(shape)
        sh.insert(0, 1)
        return self.read_3d_input_files(index, varname, sh)[0]


    def testHalfLevelsWithBounds(self):
        try:
            # PARAMETERS
            ntimes = 2
            lon = 4
            ...
            for m in varin3d:
                print "3d VAR:",m
                var3d_ids.append(
                    cmor.variable(table_entry=self.specs[m]["entry"],
                                units=self.specs[m]["units"],
                                axis_ids=[itim, ilev_half, ilat, ilon],
                                missing_value=1.e28,
                                positive=self.specs[m]["positive"],
                                original_name=m)
                )
            print "Ok now writing",var3d_ids, ntimes
            for index in range(ntimes):
                tim_array, bnds_tim = self.read_time(index)
                for i, varname in enumerate(varin3d):
                    data = self.read_3d_input_files(index, varname,(lev,lat,lon))
                    ...
                    print("Passed write")
                    # PSURF
                    data = self.read_2d_input_files(index,"PSURF",(lat,lon))
