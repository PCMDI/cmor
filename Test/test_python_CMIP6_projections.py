...
class TestCase(unittest.TestCase):
    ...
    def testCMIP6(self):
        ...
        #
        coordVals = numpy.arange(-533750.0, 533750.0 + 2500.0, 2500.0, numpy.float32)
        ...
        #
        xgrid, ygrid = numpy.broadcast_arrays(numpy.expand_dims(coordVals, 0), numpy.expand_dims(coordVals, 1))
        ...
        #
        entry = { 'table_entry' : 'y', 'units' : 'm', 'coord_vals' : coordVals, 'cell_bounds' : coordBnds }

        axis_ygre = cmor.axis(**entry)
        
        entry = { 'table_entry' : 'x', 'units' : 'm', 'coord_vals' : coordVals, 'cell_bounds' : coordBnds }

        axis_xgre = cmor.axis(**entry)
        
        # Create the grid
        #
        grid_id = cmor.grid(axis_ids = [axis_ygre, axis_xgre], latitude = latgrid, longitude = longrid)
        ...
        ierr = cmor.set_grid_mapping(grid_id, 'polar_stereographic', param_dict) 
