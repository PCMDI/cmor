...
LICENSE_INFO = {
    "license_template":
        ("CMIP7 data produced by <institution_id> is licensed under a <license_type> "
        "License (<license_url>). Consult https://pcmdi.llnl.gov/CMIP6/TermsOfUse "
...
class TestLicenseTemplate(unittest.TestCase):
    def setUp(self):
        """
        ...
        # Add license template to CV
        cv_dict: dict
        with open("TestTables/CMIP7_CV.json", "r") as cv_file_handle:
            cv_dict = json.load(cv_file_handle)

        with open("TestTables/CMIP7_CV_lic_template.json", "w") as cv_file_handle:
            cv_dict["CV"]["license"] = LICENSE_INFO
            json.dump(cv_dict, cv_file_handle, sort_keys=True, indent=4)

        # Define dataset using DATASET_INFO
        with open("Test/input_lic_template.json", "w") as input_file_handle:
            DATASET_INFO["_controlled_vocabulary_file"] = "TestTables/CMIP7_CV_lic_template.json"
            json.dump(DATASET_INFO, input_file_handle, sort_keys=True, indent=4)

        # read dataset info
        error_flag = cmor.dataset_json("Test/input_lic_template.json")
        if error_flag:
    ...
    def test_license_template(self):
        ...
        license_type = LICENSE_INFO["license_id"]["CC0 1.0"]["license_type"]
        license_url = LICENSE_INFO["license_id"]["CC0 1.0"]["license_url"]
        license = \
        (f"CMIP7 data produced by {institution_id} is licensed under a {license_type} "
        f"License ({license_url}). Consult https://pcmdi.llnl.gov/CMIP6/TermsOfUse "
        "for terms of use governing CMIP7 output, including citation requirements "
        "and proper acknowledgment. The data producers and data providers make no "
        "warranty, either express or implied, including, but not limited to, "
        "warranties of merchantability and fitness for a particular purpose. All "
