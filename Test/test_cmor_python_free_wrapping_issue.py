...
class TestCase(base_test_cmor_python.BaseCmorTest):
    ...
    def save(self, opts, threeD=True):
        try:
    ...
    def testFreeWrappingIssue(self):
        opts = {'cmip_table': 'CMIP6_Amon.json',
                'table_path': self.tabledir}
        try:
            self.save(opts, threeD=False)
        except BaseException:
            raise

    def testFreeWrappingIssue3D(self):
        opts = {'cmip_table': 'CMIP6_Amon.json',
                'table_path': self.tabledir}
        try:
            self.save(opts, threeD=True)
        except BaseException:
            raise
