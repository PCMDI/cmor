...
include_dirs.append(os.path.join("/usr/local/include","json-c"))
libraries = []

for st in ["-L/software/run/uvcdatall/Externals/lib -lnetcdf", "-L/usr/local/lib", "-I/software/run/uvcdatall/Externals/include -I/software/run/uvcdatall/Externals/include -I/software/run/uvcdatall/Externals/lib/libffi-3.1/include", "-ludunits2", "-ljson-c", "", "-luuid"]:
   sp = st.strip().split()
   for s in sp:
      if s[:2]=='-L':
...
setup (name = "CMOR",
       version='3.0',
       author='Charles Doutriaux, modified: Denis Nadeau',
