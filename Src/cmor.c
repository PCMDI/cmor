...
/*                                                                        */
/**************************************************************************/
int cmor_check_expt_id( char *expt_id, int table_id,
			char *gbl_lng, char *gbl_sht ) {
    int i, j, k, l, m;
    char msg[CMOR_MAX_STRING];
	...
	if( ( l > 4 ) && ( k > 4 ) ) {
	    strncpy( msg, &cmor_tables[table_id].expt_ids[i]
		     [strlen( cmor_tables[table_id].expt_ids[i] ) - 4],
		     4 );
	    msg[4] = '\0';
	} else {
	    strcpy( msg, "nope" );
	}
	if( strcmp( msg, "XXXX" ) == 0 ) {
/* -------------------------------------------------------------------- */
/*       we have one of these weird expt nms thing...                   */
/*       ok now we can compare                                          */
/* -------------------------------------------------------------------- */
	    if( ( strncmp( cmor_tables[table_id].expt_ids[i], expt_id,
			   l - 4 ) == 0 ) ||
		( strncmp( cmor_tables[table_id].sht_expt_ids[i], expt_id,
			   m - 4 ) == 0 ) ) {
		j = 1;
		l = strlen( cmor_tables[table_id].expt_ids[i] );
		strncpy( ctmp, cmor_tables[table_id].expt_ids[i], l - 4 );
/* -------------------------------------------------------------------- */
/*      but we need to replace the last 4 char                          */
/*      from X to the actual 4 dgits                                    */
/* -------------------------------------------------------------------- */

/* -------------------------------------------------------------------- */
/*      ok the sht_Exp id with the right digits                         */
/* -------------------------------------------------------------------- */
		strncpy( &ctmp[l - 4], &expt_id[k - 4], 4 );
		ctmp[l] = '\0';

/* -------------------------------------------------------------------- */
/*      Ok and now use this to reset the global attribute               */
/* -------------------------------------------------------------------- */
		cmor_set_cur_dataset_attribute_internal( gbl_lng, ctmp,
							 0 );

		l = strlen( cmor_tables[table_id].sht_expt_ids[i] );
		strncpy( ctmp, cmor_tables[table_id].sht_expt_ids[i],
			 l - 4 );
		strncpy( &ctmp[l - 4], &expt_id[k - 4], 4 );
		ctmp[l] = '\0';
...
/*      Ok and now use this to reset the global attribute               */
/* -------------------------------------------------------------------- */
		cmor_set_cur_dataset_attribute_internal( gbl_sht, ctmp,
							 1 );
		strncpy( expt_id, ctmp, CMOR_MAX_STRING );
		break;
	    }
	} else if( strcmp( msg, "DDHH" ) == 0 ) {
	    if( ( strncmp( cmor_tables[table_id].expt_ids[i],
			   expt_id,
			   l - 10 ) == 0 ) ||
		( strncmp( cmor_tables[table_id].sht_expt_ids[i], expt_id,
			   m - 10 ) == 0 ) ) {
...
/* -------------------------------------------------------------------- */
/*      but we need to replace the last 10 char f                       */
/*      rom X to the actual 10 dgits                                    */
/* -------------------------------------------------------------------- */
		strncpy( &ctmp[l - 10], &expt_id[k - 10], 10 );
		ctmp[l] = '\0';
/* -------------------------------------------------------------------- */
/*       Ok and now use this to reset the global attribute              */
/* -------------------------------------------------------------------- */
	    ...
	    if( ( strncmp( cmor_tables[table_id].expt_ids[i],
			   expt_id, CMOR_MAX_STRING ) == 0 )
		|| ( strncmp( cmor_tables[table_id].sht_expt_ids[i],
			      expt_id, CMOR_MAX_STRING ) == 0 ) ) {
		j = 1;
		cmor_set_cur_dataset_attribute_internal( gbl_lng,
							 cmor_tables
							 [table_id].expt_ids
							 [i], 0 );
		cmor_set_cur_dataset_attribute_internal( gbl_sht,
							 cmor_tables
							 [table_id].sht_expt_ids
							 [i], 1 );
...
void cmor_handle_error( char error_msg[CMOR_MAX_STRING], int level ) {
	...
	if( CMOR_VERBOSITY != CMOR_QUIET ) {
	    ...
	    snprintf( msg, CMOR_MAX_STRING, "! Warning: %s  !",
		      error_msg );
	}
    } else {
	cmor_nerrors++;
...
/* -------------------------------------------------------------------- */
	if( cmor_have_NetCDF363(  ) != 0 ) {
	    sprintf( msg,
		     "You are using an older version of NetCDF3 (%s), you need 3.6.3",
		     nc_inq_libvers(  ) );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
	...
	if( cmor_have_NetCDF41min(  ) != 0 ) {
	    sprintf( msg,
		     "You are using a wrong version of NetCDF4 (%s), you need 4.1",
		     nc_inq_libvers(  ) );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
    } else {
	sprintf( msg,
		 "You are using a wrong version of NetCDF (%s), you need 3.6.3 or 4.1",
		 nc_inq_libvers(  ) );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
...
/* -------------------------------------------------------------------- */
	    ...
	    snprintf( msg, CMOR_MAX_STRING,
		      "%s_%.4i-%.2i-%.2iT%.2i:%.2i:%.2i", tmplogfile,
	    ...
	    snprintf( msg2, CMOR_MAX_STRING,
		      "Logfile %s already existed. Renamed to: %s",
		      tmplogfile, msg );
	    output_logfile = NULL;
	...
	if( *mode != CMOR_EXIT_ON_WARNING && *mode != CMOR_EXIT_ON_MAJOR
	    ...
	    snprintf( msg, CMOR_MAX_STRING,
		      "exit mode can be either CMOR_EXIT_ON_WARNING CMOR_NORMAL or CMOR_EXIT_ON_MAJOR" );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
	...
	if( *netcdf != CMOR_PRESERVE_4 && *netcdf != CMOR_APPEND_4
	    ...
	    snprintf( msg, CMOR_MAX_STRING,
		      "file mode can be either CMOR_PRESERVE, CMOR_APPEND, CMOR_REPLACE, CMOR_PRESERVE_4, CMOR_APPEND_4, CMOR_REPLACE_4, CMOR_PRESERVE_3, CMOR_APPEND_3 or CMOR_REPLACE_3" );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
    ...
    if( ( ( CMOR_NETCDF_MODE == CMOR_PRESERVE_4 )
	  ...
	  || ( CMOR_NETCDF_MODE == CMOR_APPEND_4 ) )
	&& ( USE_NETCDF_4 == 0 ) ) {
	sprintf( msg,
...
/* -------------------------------------------------------------------- */
    ...
    if( ut_get_status(  ) != UT_SUCCESS ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "Udunits: Error reading units system" );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
    ut_set_error_message_handler( ut_ignore );
    if( newequnit != NULL )
    ...
    if( myutstatus != UT_SUCCESS ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "Udunits: Error mapping dimless unit" );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
    if( perunit != NULL )
	ut_free( perunit );
    ...
    if( myutstatus != UT_SUCCESS ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "Udunits: Error creating percent unit" );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
    myutstatus = ut_map_name_to_unit( "%", UT_ASCII, perunit );
    if( myutstatus != UT_SUCCESS ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "Udunits: Error mapping percent unit" );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
/* -------------------------------------------------------------------- */
/*      initialized dataset                                             */
...
/* -------------------------------------------------------------------- */
    ...
    free( myuuid_str );
    uuid_destroy( myuuid );
    strncpy( cmor_current_dataset.associated_file_name, "",
	     CMOR_MAX_STRING );
...
/************************************************************************/
int cmor_put_nc_num_attribute( int ncid,
			       int nc_var_id,
			       char *name,
			       char type, double value, char *var_name ) {
    char msg[CMOR_MAX_STRING];
    int ierr;
    ...
    if( type == 'i' ) {
	ierr =
	    nc_put_att_double( ncid, nc_var_id, name, NC_INT, 1, &value );
    } else if( type == 'l' ) {
	ierr =
	    nc_put_att_double( ncid, nc_var_id, name, NC_INT, 1, &value );
    } else if( type == 'f' ) {
	ierr =
	    nc_put_att_double( ncid, nc_var_id, name, NC_FLOAT, 1,
			       &value );
    } else if( type == 'd' ) {
	ierr =
	    nc_put_att_double( ncid, nc_var_id, name, NC_DOUBLE, 1,
			       &value );
    }
    if( ierr != NC_NOERR ) {
	snprintf( msg,
...
/************************************************************************/
int cmor_put_nc_char_attribute( int ncid, int nc_var_id, char *name,
	...
	if( ierr != NC_NOERR ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "NetCDF Error (%i: %s) setting attribute: '%s' on variable (%s)",
		      ierr, nc_strerror( ierr ), name, var_name );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
    }
...
/* -------------------------------------------------------------------- */
	...
	snprintf( msg, CMOR_MAX_STRING,
		  "you are trying to set dataset attribute: %s this must be set via a call to cmor_dataset or is set internally by CMOR via the tables",
		  name );
	cmor_handle_error( msg, CMOR_NORMAL );
	cmor_pop_traceback(  );
	return 1;
    ...
    cmor_pop_traceback(  );
    return cmor_set_cur_dataset_attribute_internal( name, value,
						    optional );
}
...
/************************************************************************/
int cmor_set_cur_dataset_attribute_internal( char *name,
	...
	snprintf( msg, CMOR_MAX_STRING,
		  "CMOR Dataset error, attribute name: %s; length (%i) is greater than limit: %i",
	...
	if( optional == 1 ) {
	    ...
	    snprintf( msg, CMOR_MAX_STRING,
		      "CMOR Dataset error, required attribute %s was not passed or blanked",
		      name );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	    cmor_pop_traceback(  );
	    return 1;
	...
	sprintf( msg,
		 "Setting dataset attribute: %s, we already have %i elements set which is the max, this element won't be set",
		 name, CMOR_MAX_ELEMENTS );
	cmor_handle_error( msg, CMOR_NORMAL );
	cmor_pop_traceback(  );
	return 1;
    }
    strncpy( cmor_current_dataset.attributes_names[n], msg,
	     CMOR_MAX_STRING );
    cmor_trim_string( value, msg );
    strncpytrim( cmor_current_dataset.attributes_values[n], msg,
		 CMOR_MAX_STRING );
    cmor_current_dataset.nattributes += 1;
    cmor_pop_traceback(  );
    return 0;
...
int cmor_has_cur_dataset_attribute( char *name ) {
    ...
    if( ( int ) strlen( name ) > CMOR_MAX_STRING ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "CMOR Dataset: attribute name (%s) length (%i) is greater than limit: %i",
		  name, ( int ) strlen( name ), CMOR_MAX_STRING );
	cmor_handle_error( msg, CMOR_NORMAL );
	cmor_pop_traceback(  );
	return 1;
...
/* -------------------------------------------------------------------- */
	if( getuid(  ) != 0 ) {
		...
		sprintf( msg,
			 "You defined your output directory to be: '%s', but you do not have read/write permissions on it",
			 cmor_current_dataset.outpath );
		cmor_handle_error( msg, CMOR_CRITICAL );
	    } else {
		fclose( test_file );
	...
	sprintf( msg,
		 "You defined your output directory to be: '%s', but this directory does not exist",
		 cmor_current_dataset.outpath );
	cmor_handle_error( msg, CMOR_CRITICAL );
    } else if( errno == EACCES ) {
	sprintf( msg,
		 "You defined your output directory to be: '%s', but we cannot access it, please check permissions",
		 cmor_current_dataset.outpath );
	cmor_handle_error( msg, CMOR_CRITICAL );
    }
...
/* -------------------------------------------------------------------- */
	...
	if( found == 1 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "You passed calendar: %s therefore we will ignore any user defined value you also set for month_lentgths and leap_months",
		      calendar );
	    cmor_handle_error( msg, CMOR_WARNING );
	} else {
	    snprintf( msg, CMOR_MAX_STRING,
		      "You defined a non_standard calendar while this used to be ok in CMOR version 1 it is no longer supported in this version, please contact us at: cmor@lists.llnl.gov so we can work on fixing this issue" );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
    } else if( found == 0 ) {
	snprintf( msg, CMOR_MAX_STRING,
    ...
    if( realization < 0 ) {
	snprintf( msg, CMOR_MAX_STRING,
		  "Error realization number is negative, expected a positive number or 0 (i.e. ignored)" );
	cmor_handle_error( msg, CMOR_CRITICAL );
	cmor_pop_traceback(  );
	return 1;
    } else {
	...
	cmor_set_cur_dataset_attribute_internal( "physics_version", msg,
						 0 );
    } else {
	cmor_set_cur_dataset_attribute_internal( "physics_version", "1",
						 0 );
    }
...
/* -------------------------------------------------------------------- */
	    ...
	    if( found == 0 ) {
		snprintf( msg, CMOR_MAX_STRING,
			  "could not find the zfactor variable: %s, please define it first, while defining zfactors for variable %s (table %s)",
...
/* -------------------------------------------------------------------- */
			if( cmor_axes[cmor_vars[var_id].axes_ids[m]].axis
				...
				if( cmor_vars[var_id].associated_ids[ia] ==
				    -1 ) {
				    cmor_vars[var_id].associated_ids[ia] =
					l;
				    break;
				}
			    }
		...
		if( found == 0 ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "variable \"%s\" (table: %s) has axis \"%s\" defined with formula terms, but term \"%s\" depends on axis \"%s\" which is not part of the variable",
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id,
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NC Error (%i: %s) for variable %s (table %s) error defining zfactor var: %i (%s)",
...
/* -------------------------------------------------------------------- */
		if( ( CMOR_NETCDF_MODE != CMOR_REPLACE_3 )
			...
			ics =
			    cmor_tables[cmor_vars[l].
					ref_table_id].vars[cmor_vars[l].
							   ref_var_id].
			    shuffle;
			icd =
			    cmor_tables[cmor_vars[l].
			...
			icdl =
			    ...
			    deflate_level;
			ierr =
			    nc_def_var_deflate( ncid,
						nc_zfactors[lnzfactors],
						ics, icd, icdl );
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NCError (%i: %s) defining compression parameters for zfactor variable %s for variable '%s' (table %s)",
				      ierr, nc_strerror( ierr ),
				      cmor_vars[l].id,
				      cmor_vars[var_id].id,
...
/* -------------------------------------------------------------------- */
			ierr =
			    cmor_convert_string_to_list( cmor_vars
							 ...
							 &nelts );
			ierr =
			    nc_put_att_int( ncid, nc_zfactors[lnzfactors],
					    "flag_values", NC_INT, nelts,
					    int_list );
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NetCDF Error (%i: %s) setting flags numerical attribute on zfactor variable %s for variable %s (table %s)",
				      ierr, nc_strerror( ierr ),
				      cmor_vars[l].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
			...
			ierr =
			    cmor_put_nc_char_attribute( ncid,
							nc_zfactors
							[lnzfactors],
							cmor_vars
							[l].attributes[k],
			...
			ierr =
			    cmor_put_nc_num_attribute( ncid,
						       nc_zfactors
						       [lnzfactors],
						       cmor_vars
						       [l].attributes[k],
...
/* -------------------------------------------------------------------- */
		...
		if( ( CMOR_NETCDF_MODE != CMOR_REPLACE_3 )
			...
			deflate_level;
		    ierr =
			nc_def_var_deflate( ncid, nc_zfactors[lnzfactors],
					    ics, icd, icdl );
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NCError (%i: %s) defining compression parameters for zfactor variable %s for variable '%s' (table %s)",
...
/* -------------------------------------------------------------------- */
	...
	[cmor_axes[cmor_vars[var_id].axes_ids[i]].
	 ref_table_id].axes[cmor_axes[cmor_vars[var_id].axes_ids[i]].
			    ref_axis_id].stored_direction == 'd' ) {
/* -------------------------------------------------------------------- */
/*      decrease stuff                                                  */
/* -------------------------------------------------------------------- */
	if( cmor_axes[cmor_vars[var_id].axes_ids[i]].values[1] >
	    cmor_axes[cmor_vars[var_id].axes_ids[i]].values[0] )
	    doflip = 1;
    } else {
	if( cmor_axes[cmor_vars[var_id].axes_ids[i]].values[1] <
	    cmor_axes[cmor_vars[var_id].axes_ids[i]].values[0] )
...
/* -------------------------------------------------------------------- */
	...
	for( j = 0;
	     j < cmor_axes[cmor_vars[var_id].axes_ids[i]].length / 2;
	     j++ ) {
		...
		cmor_axes[cmor_vars[var_id].
			  axes_ids[i]].values[cmor_axes[cmor_vars[var_id].
							axes_ids[i]].
					      length - 1 - j];
	    cmor_axes[cmor_vars[var_id].
		      axes_ids[i]].values[cmor_axes[cmor_vars[var_id].
		...
		cmor_vars[k].values[cmor_axes
				    [cmor_vars[var_id].axes_ids[i]].
				    length - 1 - j];
	    cmor_vars[k].
		values[cmor_axes[cmor_vars[var_id].axes_ids[i]].length -
		       1 - j] = tmp;
	    if( b != NULL ) {
		tmp = cmor_vars[l].values[j];
		cmor_vars[l].values[j] =
		    cmor_vars[l].values[cmor_axes
					[cmor_vars[var_id].
					 axes_ids[i]].length - 1 - j];
		cmor_vars[l].values[cmor_axes
				    [cmor_vars[var_id].
				     axes_ids[i]].length - 1 - j] = tmp;
	    }
	}
	if( cmor_axes[cmor_vars[var_id].axes_ids[i]].bounds != NULL ) {
		...
		cmor_axes[cmor_vars[var_id].axes_ids[i]].bounds[j] =
		    cmor_axes[cmor_vars[var_id].
			      axes_ids[i]].bounds[cmor_axes[cmor_vars
							    [var_id].
							    axes_ids
							    [i]].length *
						  2 - 1 - j];
		cmor_axes[cmor_vars[var_id].
			  axes_ids[i]].bounds[cmor_axes[cmor_vars[var_id].
							axes_ids[i]].length
					      * 2 - 1 - j] = tmp;
		tmp = cmor_vars[k].values[j];
		cmor_vars[k].values[j] =
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_vars[varid].initialized == -1 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "You are trying to write variable \"%s\" in association with variable \"%s\" (table %s), but you you need to write the associated variable first in order to initialize the file and dimensinos",
		      cmor_vars[varid].id, cmor_vars[var_id].id,
		      cmor_tables[cmor_vars[var_id].
				  ref_table_id].table_id );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
...
/* -------------------------------------------------------------------- */
	ierr =
	    nc_inq_varid( cmor_vars[varid].initialized,
			  cmor_vars[var_id].id,
			  &cmor_vars[var_id].nc_var_id );
	if( ierr != NC_NOERR ) {
	    sprintf( msg,
		     "Could not find variable: '%s' (table: %s) in file of associated variable: '%s'",
		     cmor_vars[var_id].id,
		     cmor_tables[cmor_vars[var_id].ref_table_id].table_id,
		     cmor_vars[*refvar].id );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	}
	cmor_vars[var_id].ntimes_written =
	    cmor_vars[varid].ntimes_written - ntimes_passed;
    }
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_vars[varid].itype != type ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "You defined variable \"%s\" (table %s) with a missing value of type \"%c\", but you are now writing data of type: \"%c\" this may lead to some spurious handling of the missing values",
		      cmor_vars[varid].id,
		      cmor_tables[cmor_vars[var_id].ref_table_id].table_id,
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_has_cur_dataset_attribute( "forcing" ) == 0 ) {
	    ...
	    cmor_check_forcing_validity( cmor_vars[var_id].ref_table_id,
					 ctmp2 );
	}
...
/* -------------------------------------------------------------------- */
	strncpy( ctmp2,
		 cmor_tables[cmor_vars[var_id].ref_table_id].product,
		 CMOR_MAX_STRING );
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_check_expt_id
	    ( ctmp2, cmor_vars[var_id].ref_table_id, "experiment",
	      "experiment_id" ) != 0 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "Invalid dataset experiment id: %s, while writing variable %s, check against table: %s",
...
/* -------------------------------------------------------------------- */
			if( cmor_tables
			    ...
			    sprintf( msg,
				     "You passed '%s' as file_suffix while writing variable %s (table %s), suffix are not allowed in CMOR2.0 and newer. Were you trying to append to a non-existing file?",
...
/* -------------------------------------------------------------------- */
			...
			2. ) {
			sprintf( msg,
				 "Suffix are not allowed in CMOR 2.0 and greater, suffix %s encountered while writing variable %s (table %s)",
...
/* -------------------------------------------------------------------- */
	...
	for( i = 0; i < strlen( cmor_vars[var_id].id ); i++ ) {
		...
		snprintf( outname, CMOR_MAX_STRING,
			  "var_id cannot contain %c you passed: %s (table: %s). Please check your input tables\n",
	...
	if( cmor_has_cur_dataset_attribute( "model_id" ) == 0 ) {
	    ...
	    substitute_chars_with_hyphens( msg, ctmp5, "model_id",
					   var_id );
	    strncat( outname, "_", CMOR_MAX_STRING - strlen( outname ) );
...
/* -------------------------------------------------------------------- */
	if( isfixed == 1 ) {
	    strncat( outname, "r0i0p0",
		     CMOR_MAX_STRING - strlen( outname ) );
	    origRealization = cmor_current_dataset.realization;
	    cmor_current_dataset.realization = 0;
	    cmor_set_cur_dataset_attribute_internal( "physics_version",
						     "0", 0 );
	    cmor_set_cur_dataset_attribute_internal
		( "initialization_method", "0", 0 );
	} else {
	    snprintf( msg, CMOR_MAX_STRING, "r%d",
...
/* -------------------------------------------------------------------- */
	    if( cmor_has_cur_dataset_attribute( "initialization_method" )
		== 0 ) {
		cmor_get_cur_dataset_attribute( "initialization_method",
						ctmp2 );
		sscanf( ctmp2, "%i", &j );
...
/* -------------------------------------------------------------------- */
	...
	strncpytrim( cmor_vars[var_id].base_path, outname,
		     CMOR_MAX_STRING );
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
		...
		if( msg[i] == '_' ) {
			...
			snprintf( ctmp2, CMOR_MAX_STRING,
				  "While writing variable %s (table %s), suffix cannot contain the '_' character, you passed: %s. Is it possible you are trying to append to a file and the file is not actually here?",
				  cmor_vars[var_id].id,
				  cmor_tables[cmor_vars
					      [var_id].ref_table_id].
				  table_id, msg );
			...
			snprintf( ctmp2, CMOR_MAX_STRING,
				  "suffix cannot contain the '_' character, you passed: %s, while writing variable %s (table: %s)",
...
/* -------------------------------------------------------------------- */
	if( appending_to[0] != '\0' ) {
		...
		if( rename( appending_to, outname ) != 0 ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "could not rename your existing file: %s to temporary file: %s",
	...
	strncpytrim( cmor_vars[var_id].current_path, outname,
		     CMOR_MAX_STRING );
/* -------------------------------------------------------------------- */
/*      Decides NetCDF mode                                             */
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
	    ...
	    if( fperr != NULL ) {
		snprintf( msg, CMOR_MAX_STRING,
			  "Output file ( %s ) already exists, remove file or use CMOR_REPLACE or CMOR_APPEND for CMOR_NETCDF_MODE value in cmor_setup",
			  outname );
		cmor_handle_error( msg, CMOR_CRITICAL );
	    }
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) looking for time dimension in file: %s",
			      ierr, nc_strerror( ierr ), outname );
		    cmor_handle_error( msg, CMOR_CRITICAL );
		}
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) looking for time dimension length in file: %s",
			      ierr, nc_strerror( ierr ), outname );
		    cmor_handle_error( msg, CMOR_CRITICAL );
		}
		ierr =
		    nc_inq_varid( ncid, cmor_vars[var_id].id,
				  &cmor_vars[var_id].nc_var_id );
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) looking for variable '%s' in file: %s",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id, outname );
		    cmor_handle_error( msg, CMOR_CRITICAL );
		}
		ierr =
		    nc_inq_varid( ncid, "time",
				  &cmor_vars[var_id].time_nc_id );
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) looking for time of variable '%s' in file: %s",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id, outname );
		    cmor_handle_error( msg, CMOR_CRITICAL );
		}
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) looking for time bounds of variable '%s' in file: %s",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id, outname );
		    cmor_handle_error( msg, CMOR_WARNING );
		    ierr = NC_NOERR;
...
/* -------------------------------------------------------------------- */
		    ...
		    starts[1] = 1;
		    ierr =
			nc_get_var1_double( ncid,
					    cmor_vars
					    [var_id].time_bnds_nc_id,
					    &starts[0],
					    &cmor_vars
					    [var_id].last_bound );
		    starts[1] = 0;
...
/* -------------------------------------------------------------------- */
	if( cmor_current_dataset.initiated == 0 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "you need to initialize the dataset by calling cmor_dataset before calling cmor_write" );
	    cmor_handle_error( msg, CMOR_NORMAL );
	    cmor_pop_traceback(  );
	    return 1;
	}
...
/* -------------------------------------------------------------------- */
	...
	if( strcmp( cmor_tables[cmor_vars[var_id].ref_table_id].URL, "" )
	    == 0 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "Your table (%s) does not contain a reference URL, please consider adding it",
		      cmor_tables[cmor_vars[var_id].
				  ref_table_id].table_id );
	    cmor_handle_error( msg, CMOR_WARNING );
...
/* -------------------------------------------------------------------- */
	    ...
	    strncpy( ctmp3,
		     cmor_tables[cmor_vars[var_id].ref_table_id].URL,
		     CMOR_MAX_STRING );
	    strcpy( ctmp, "baseURL: " );
...
/* -------------------------------------------------------------------- */
	if( cmor_tables[cmor_vars[var_id].ref_table_id].vars
	    [cmor_vars[var_id].ref_var_id].realm[0] != '\0' ) {
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
		...
		if( cmor_tables[cmor_vars[var_id].ref_table_id].vars
		    ...
		    ctmp3[i] =
			cmor_tables[cmor_vars[var_id].
				    ref_table_id].vars[cmor_vars[var_id].
						       ref_var_id].
			realm[i];
		    ctmp3[i + 1] = '\0';
		} else {
...
/* -------------------------------------------------------------------- */
	    strncattrim( ctmp2,
			 cmor_tables[cmor_vars[var_id].ref_table_id].realm,
...
/* -------------------------------------------------------------------- */
	...
	if( ierr != NC_NOERR && ierr != NC_EINDEFINE ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "NetCDF Error (%i: %s) putting metadata file (%s) in def mode, nc file id was: %i, you were writing variable %s (table: %s)",
		      ierr, nc_strerror( ierr ),
...
/* -------------------------------------------------------------------- */
	...
	if( did_history == 0 ) {
	    snprintf( ctmp, CMOR_MAX_STRING,
		      "%s CMOR rewrote data to comply with CF standards and %s requirements.",
		      msg,
		      cmor_tables[cmor_vars[var_id].
				  ref_table_id].project_id );
	...
	snprintf( msg, CMOR_MAX_STRING, "CF-%.1f",
		  cmor_tables[cmor_vars[var_id].ref_table_id].cf_version );
	cmor_set_cur_dataset_attribute_internal( "Conventions", msg, 0 );
	cmor_set_cur_dataset_attribute_internal( "project_id",
						 cmor_tables[cmor_vars
							     [var_id].ref_table_id].project_id,
						 0 );
	snprintf( msg, CMOR_MAX_STRING, "Table %s (%s) ",
		  cmor_tables[cmor_vars[var_id].ref_table_id].table_id,
		  cmor_tables[cmor_vars[var_id].ref_table_id].date );
	for( i = 0; i < 16; i++ )
	    sprintf( &ctmp[2 * i], "%02x",
		     cmor_tables[cmor_vars[var_id].ref_table_id].md5[i] );
	ctmp[32] = '\0';
	strcat( msg, ctmp );
	...
	snprintf( msg, CMOR_MAX_STRING,
		  ...
		  cmor_tables[cmor_vars[var_id].ref_table_id].project_id,
		  ctmp2 );
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_tables[cmor_vars[var_id].ref_table_id].cf_version >
	    afloat ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "Your table (%s) claims to enforce CF version %f but this version of the library is designed for CF up to: %i.%i, you were writing variable: %s",
		      cmor_tables[cmor_vars[var_id].ref_table_id].table_id,
		      cmor_tables[cmor_vars[var_id].
				  ref_table_id].cf_version,
...
/* -------------------------------------------------------------------- */
		...
		if( strcmp( msg, ctmp ) == 0 ) {
		    sprintf( ctmp,
			     "Your parent_experiment id matches your current experiment_id, they are both set to: %s; you were writing variable %s (table: %s)",
			     msg, cmor_vars[var_id].id,
			     cmor_tables[cmor_vars[var_id].
					 ref_table_id].table_id );
...
/* -------------------------------------------------------------------- */
			if( cmor_check_expt_id
			    ( msg, cmor_vars[var_id].ref_table_id,
			      "parent_experiment",
			      "parent_experiment_id" ) != 0 ) {
			    snprintf( ctmp, CMOR_MAX_STRING,
				      "Invalid dataset parent experiment id: %s, check against table: %s, you were writing variable: %s",
				      msg,
				      cmor_tables[cmor_vars
						  [var_id].ref_table_id].
				      table_id, cmor_vars[var_id].id );
			    cmor_handle_error( ctmp, CMOR_NORMAL );
			    cmor_pop_traceback(  );
			    return 1;
			}
		...
		if( cmor_has_cur_dataset_attribute( "branch_time" ) == 0 ) {
		    ...
		    if( tmps[0] != 0. ) {
			sprintf( msg,
				 "when dataset attribute parent_experiment_id is set to N/A, branch_time must be 0., you passed: %lf, we are resetting to 0. for variable %s (table: %s)",
	...
	cmor_has_required_global_attributes( cmor_vars
					     [var_id].ref_table_id );
	...
	if( strcmp
		...
		snprintf( msg, CMOR_MAX_STRING,
			  "while writing variable %s (table: %s), source attribute does not start with 'model_id', it should start with: %s",
			  cmor_vars[var_id].id,
			  cmor_tables[cmor_vars[var_id].
				      ref_table_id].table_id, ctmp5 );
		cmor_handle_error( msg, CMOR_CRITICAL );
	    }
	}
...
/* -------------------------------------------------------------------- */
	...
	fmt = UUID_FMT_STR;
	uuid_export( myuuid, fmt, &myuuid_str, &uuidlen );
	if( cmor_tables[cmor_vars[var_id].ref_table_id].tracking_prefix !=
	    '\0' ) {
	    strncpy( cmor_current_dataset.tracking_id,
		     cmor_tables[cmor_vars[var_id].
				 ref_table_id].tracking_prefix,
		     CMOR_MAX_STRING );
	...
	for( i = 0; i < cmor_current_dataset.nattributes; i++ ) {
		...
		if( ( strcmp( cmor_current_dataset.attributes_names[i], "initialization_method" ) == 0 ) || ( strcmp( cmor_current_dataset.attributes_names[i], "physics_version" ) == 0 ) ) {	/* these two are actually int not char */
		    sscanf( cmor_current_dataset.attributes_values[i],
			    "%i", &itmp2 );
		    ierr =
			nc_put_att_int( ncid, NC_GLOBAL,
					cmor_current_dataset.attributes_names
					[i], NC_INT, 1, &itmp2 );
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF error (%i: %s) for variable %s (table: %s) writing global att: %s (%s)",
				  ierr, nc_strerror( ierr ),
				  cmor_vars[var_id].id,
				  cmor_tables[cmor_vars
					      [var_id].ref_table_id].
			...
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NetCDF error (%i: %s) for variable %s (table: %s) writing global att to metafile: %s (%s)",
				      ierr, nc_strerror( ierr ),
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
						  [var_id].ref_table_id].
...
/* -------------------------------------------------------------------- */
		    ...
		    if( ncid != ncafid ) {
			ierr =
			    nc_put_att_text( ncafid, NC_GLOBAL,
					     cmor_current_dataset.attributes_names
					     [i], itmp2,
					     cmor_current_dataset.attributes_values
					     [i] );
...
/* -------------------------------------------------------------------- */
	ierr =
	    nc_put_att_int( ncid, NC_GLOBAL, "realization", NC_INT, 1,
			    &cmor_current_dataset.realization );
	if( ierr != NC_NOERR ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "NetCDF error (%i: %s) writing variable %s (table: %s) global att realization (%i)",
...
/* -------------------------------------------------------------------- */
	...
	ierr =
	    nc_put_att_text( ncid, NC_GLOBAL, "cmor_version",
			     strlen( msg ) + 1, msg );
	if( ierr != NC_NOERR ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "NetCDF error (%i: %s) writing variable %s (table: %s) global att cmor_version (%f)",
		      ierr, nc_strerror( ierr ), cmor_vars[var_id].id,
		      cmor_tables[cmor_vars[var_id].ref_table_id].table_id,
		      afloat );
	    cmor_handle_error( msg, CMOR_CRITICAL );
	...
	if( ncid != ncafid ) {
	    ierr =
		nc_put_att_int( ncafid, NC_GLOBAL, "realization", NC_INT,
				1, &cmor_current_dataset.realization );
	    if( ierr != NC_NOERR ) {
		snprintf( msg, CMOR_MAX_STRING,
			  "NetCDF error (%i: %s) writing variable %s (table: %s) global att realization (%i) to metafile",
			  ierr, nc_strerror( ierr ), cmor_vars[var_id].id,
			  cmor_tables[cmor_vars[var_id].
				      ref_table_id].table_id,
			  cmor_current_dataset.realization );
...
/* -------------------------------------------------------------------- */
	    j = cmor_axes[cmor_vars[var_id].axes_ids[i]].length;
	    if( ( i == 0 )
		&& ( cmor_axes[cmor_vars[var_id].axes_ids[i]].axis ==
		     'T' ) )
		j = NC_UNLIMITED;
	    if( ( cmor_axes[cmor_vars[var_id].axes_ids[i]].axis == 'X' )
		|| ( cmor_axes[cmor_vars[var_id].axes_ids[i]].axis ==
		     'Y' ) ) {
		nc_dim_chunking[i] = j;
	    } else if( cmor_axes[cmor_vars[var_id].axes_ids[i]].isgridaxis
		       == 1 ) {
		nc_dim_chunking[i] = j;
	    } else {
		nc_dim_chunking[i] = 1;
	    ...
	    if( ierr != NC_NOERR ) {
		snprintf( msg, CMOR_MAX_STRING,
			  "NetCDF error (%i:%s) for dimension definition of axis: %s (%i), for variable %i (%s, table: %s)",
			  ierr, nc_strerror( ierr ),
			  cmor_axes[cmor_vars[var_id].axes_ids[i]].id,
			  cmor_vars[var_id].axes_ids[i], var_id,
			  cmor_vars[var_id].id,
			  cmor_tables[cmor_vars[var_id].
				      ref_table_id].table_id );
		ierr = nc_enddef( ncid );
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF error (%i: %s) for dimension definition of axis: %s (%i) in metafile, variable %s (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_axes[cmor_vars[var_id].axes_ids[i]].id,
			      i, cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id );
		    cmor_handle_error( msg, CMOR_CRITICAL );
		}
...
/* -------------------------------------------------------------------- */
	...
	if( ierr != NC_NOERR ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "NC error (%i: %s), error creating bnds dimension to metafile, variable %s (table: %s)",
		      ierr, nc_strerror( ierr ), cmor_vars[var_id].id,
		      cmor_tables[cmor_vars[var_id].
				  ref_table_id].table_id );
	    cmor_handle_error( msg, CMOR_CRITICAL );
...
/* -------------------------------------------------------------------- */
		if( ( CMOR_NETCDF_MODE != CMOR_REPLACE_3 )
			...
			deflate_level;
		    ierr =
			nc_def_var_deflate( ncid, nc_vars[i], ics, icd,
					    icdl );
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
...
/* -------------------------------------------------------------------- */
			...
			icdl =
			    cmor_tables[cmor_vars[var_id].
					ref_table_id].vars[cmor_vars
							   [var_id].
							   ref_var_id].deflate_level;
			ierr =
			    nc_def_var_deflate( ncafid, nc_vars_af[i], ics,
						icd, icdl );
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NCError (%i: %s) defining compression parameters for dimension %s for variable '%s' (table: %s)",
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF error (%i: %s) for dummy 'strlen' dimension definition of axis: %s (%i) in metafile, while writing variable %s (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_axes[cmor_vars[var_id].axes_ids[i]].id,
			      i, cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
		...
		if( ncid != ncafid ) {
		    ...
		    ierr =
			nc_def_var( ncafid, ctmp, NC_CHAR, 1, &tmp_dims[0],
				    &nc_vars_af[i] );
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF Error (%i: %s) for variable %s (table: %s) error defining dim var: %i (%s) in metafile",
...
/* -------------------------------------------------------------------- */
		...
		ierr =
		    nc_def_var( ncafid, ctmp, j, 2, &dims_bnds_ids[0],
				&nc_bnds_vars[i] );
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) for variable %s (table: %s) error defining bounds dim var: %i (%s)",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id, i,
...
/* -------------------------------------------------------------------- */
		if( ( CMOR_NETCDF_MODE != CMOR_REPLACE_3 )
			...
			deflate_level;
		    ierr =
			nc_def_var_deflate( ncafid, nc_bnds_vars[i], ics,
					    icd, icdl );
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NCError (%i: %s) defining compression parameters for bounds variable %s for variable '%s' (table: %s)",
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) for variable %s (table: %s) error defining bounds attribute var: %i (%s)",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id, i,
...
/* -------------------------------------------------------------------- */
	    ...
	    for( j = 0;
		 ...
		 j++ ) {
		if( strcmp
		    ( cmor_axes[cmor_vars[var_id].axes_ids[i]].attributes
		      [j], "z_factors" ) == 0 ) {
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( ctmp, CMOR_MAX_STRING,
				  "NetCDF error (%i: %s) writing formula term att (%s) for axis %i (%s), variable %s (table: %s)",
		    ...
		    ierr =
			cmor_define_zfactors_vars( var_id, ncafid,
						   &nc_dim_af[0], msg,
						   &nzfactors,
						   &zfactors[0],
						   &nc_zfactors[0], i,
		    ...
		    n = strlen( msg ) + 1;
		    ierr =
			nc_put_att_text( ncafid, nc_bnds_vars[i],
					 "formula", n, msg );
		    cmor_get_axis_attribute( cmor_vars[var_id].axes_ids[i],
					     "standard_name", 'c', &msg );
		    n = strlen( msg );
		    ...
		    n = strlen( msg ) + 1;
		    ierr =
			nc_put_att_text( ncafid, nc_bnds_vars[i], "units",
					 n, msg );
		    /*formula terms */
		    strncpy( msg,
			     cmor_axes[cmor_vars[var_id].
			...
			nc_put_att_text( ncafid, nc_bnds_vars[i],
					 "formula_terms", n, msg );
		    ierr =
			cmor_define_zfactors_vars( var_id, ncafid, nc_dim,
						   msg, &nzfactors,
						   &zfactors[0],
						   &nc_zfactors[0], i,
						   dim_bnds );
...
/* -------------------------------------------------------------------- */
		    ...
		    if( ( cmor_tables
			  [cmor_axes
			   [cmor_vars[var_id].axes_ids[i]].ref_table_id].
			  axes[cmor_axes
			       [cmor_vars[var_id].axes_ids
				[i]].ref_axis_id].type == 'c' )
			&&
			( strcmp
			  ( cmor_axes
			    [cmor_vars[var_id].axes_ids[i]].attributes[j],
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_axes
			    [cmor_vars[var_id].
			     axes_ids[i]].attributes_type[j] == 'c' ) {
			    ierr =
				cmor_put_nc_char_attribute( ncid,
							    nc_vars[i],
							    cmor_axes
...
/* -------------------------------------------------------------------- */
	    ...
	    if( strcmp( cmor_grids[cmor_vars[var_id].grid_id].mapping, "" )
		!= 0 ) {
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
		...
		cmor_set_variable_attribute_internal( var_id,
						      ...
						      cmor_grids[cmor_vars
								 [var_id].grid_id].mapping );
		ierr =
		    nc_def_var( ncafid,
				cmor_grids[cmor_vars[var_id].
					   grid_id].mapping, NC_INT, 0,
				&nc_dims_associated[0], &m );
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF error (%i: %s) while defining associated grid mapping variable %s for variable %s (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_grids[cmor_vars[var_id].
					 grid_id].mapping,
			      cmor_vars[var_id].id,
...
/* -------------------------------------------------------------------- */
		ierr =
		    cmor_put_nc_char_attribute( ncafid, m,
						"grid_mapping_name",
						cmor_grids[cmor_vars
							   [var_id].grid_id].mapping,
						cmor_vars[var_id].id );
		for( k = 0;
		    ...
		    if( strcmp
			( cmor_grids
			  [cmor_vars[var_id].grid_id].attributes_names[k],
			  "standard_parallel1" ) == 0
			||
			strcmp( cmor_grids
				[cmor_vars[var_id].
				 grid_id].attributes_names[k],
			...
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NetCDF Error (%i: %s) writing standard_parallel to file, varialbe: %s (table: %s)",
				      ierr, nc_strerror( ierr ),
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
						  [var_id].ref_table_id].
			...
			ierr =
			    cmor_put_nc_num_attribute( ncafid, m,
						       cmor_grids[cmor_vars
								  [var_id].grid_id].attributes_names
						       [k], 'd',
						       cmor_grids[cmor_vars
								  [var_id].grid_id].attributes_values
						       [k],
						       cmor_grids[cmor_vars
								  [var_id].grid_id].mapping );
		    }
			...
			if( cmor_axes
				...
				if( cmor_has_variable_attribute
				    ...
				    cmor_cat_unique_string( msg,
							    ...
							    [cmor_grids
							     [cmor_vars
							      [var_id].grid_id].associated_variables
							     [i]].id );
				} else {
				    strncpy( msg,
...
/* -------------------------------------------------------------------- */
			...
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NetCDF error (%i: %s) while defining vertices dimension, variable %s (table: %s)",
				      ierr, nc_strerror( ierr ),
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
						  [var_id].ref_table_id].
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF error (%i: %s) while defining associated variable %s, of variable %s (table: %s)",
				  ierr, nc_strerror( ierr ),
...
/* -------------------------------------------------------------------- */
			    ...
			    if( ierr != NC_NOERR ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "NetCDF Error (%i: %s) setting flags numerical attribute on associated variable %s, for variable %s (table: %s)",
					  ierr, nc_strerror( ierr ),
					  cmor_vars[j].id,
					  cmor_vars[var_id].id,
					  cmor_tables[cmor_vars
						      [var_id].ref_table_id].table_id );
				cmor_handle_error( msg, CMOR_CRITICAL );
			    }
			    free( int_list );
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_vars[j].ndims > 0 ) {
			    ...
			    if( ierr != NC_NOERR ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "NetCDF Error (%i: %s) defining compression parameters for associated variable '%s' for variable %s (table: %s)",
					  ierr, nc_strerror( ierr ),
					  cmor_vars[j].id,
					  cmor_vars[var_id].id,
					  cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
	for( i = 0; i < CMOR_MAX_DIMENSIONS; i++ ) {
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) defining scalar variable %s for variable %s (table: %s)",
...
/* -------------------------------------------------------------------- */
		...
		if( cmor_axes[j].bounds != NULL ) {	/*yep */
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF Error (%i: %s) defining scalar bounds variable %s for variable %s (table: %s)",
				  ierr, nc_strerror( ierr ),
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) setting flags numerical attribute on variable %s (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id );
		...
		ierr =
		    cmor_put_nc_num_attribute( ncid,
					       cmor_vars[var_id].nc_var_id,
					       cmor_vars[var_id].attributes
					       [j],
					       cmor_vars
					       [var_id].attributes_type[j],
					       cmor_vars
...
/* -------------------------------------------------------------------- */
		...
		cmor_tables[cmor_vars[var_id].
			    ref_table_id].vars[cmor_vars[var_id].
					       ref_var_id].deflate_level;
	    ierr =
		nc_def_var_deflate( ncid, cmor_vars[var_id].nc_var_id, ics,
				    icd, icdl );
	    if( ierr != NC_NOERR ) {
		snprintf( msg, CMOR_MAX_STRING,
			  "NetCDF Error (%i: %s) defining compression parameters for variable '%s' (table: %s)",
			  ierr, nc_strerror( ierr ), cmor_vars[var_id].id,
			  cmor_tables[cmor_vars[var_id].
				      ref_table_id].table_id );
		cmor_handle_error( msg, CMOR_CRITICAL );
...
#define NC_CHUNKED 0
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) defining chunking parameters for variable '%s' (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id );
...
/* -------------------------------------------------------------------- */
			...
			if( k == -1 ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "could not find 'a' coeff for axis: %s, for variable %s (table: %s)",
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( k == -1 ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "could find 'b' coeff for axis: %s, for variable %s (table: %s)",
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_axes[cmor_vars[var_id].axes_ids[i]].bounds
			    ...
			    if( k == -1 ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "could not find 'a_bnds' coeff for axis: %s, for variable %s (table: %s)",
					  cmor_axes[cmor_vars
						    [var_id].axes_ids[i]].
			    ...
			    if( k == -1 ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "could find 'b_bnds' coef for axis: %s, for variable %s (table: %s)",
					  cmor_axes[cmor_vars
						    [var_id].axes_ids[i]].
					  id, cmor_vars[var_id].id,
					  cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( k == -1 ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "could not find 'ap' coeef for axis: %s, for variable %s (table: %s)",
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( k == -1 ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "could find 'b' coef for axis: %s, for variable %s (table: %s)",
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_axes[cmor_vars[var_id].axes_ids[i]].bounds
			    ...
			    if( k == -1 ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "could not find 'ap_bnds' coeff for axis: %s, for variable %s (table: %s)",
					  cmor_axes[cmor_vars
						    [var_id].axes_ids[i]].
					  id, cmor_vars[var_id].id,
					  cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( k == -1 ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "could not find 'sigma' coeef for axis: %s, for variable %s (table: %s)",
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
				      cmor_tables[cmor_vars
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_axes[cmor_vars[var_id].axes_ids[i]].bounds
			    ...
			    if( k == -1 ) {
				snprintf( msg, CMOR_MAX_STRING,
					  "could not find 'sigma_bnds' coeff for axis: %s, for variable %s (table: %s)",
					  cmor_axes[cmor_vars
						    [var_id].axes_ids[i]].
					  id, cmor_vars[var_id].id,
					  cmor_tables[cmor_vars
		...
		if( cmor_axes
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF Error (%i: %s) writing axis '%s' values for variable %s (table: %s)",
				  ierr, nc_strerror( ierr ),
				  cmor_axes[cmor_vars[var_id].
					    axes_ids[i]].id,
				  cmor_vars[var_id].id,
			...
			if( ierr != NC_NOERR ) {
			    snprintf( msg, CMOR_MAX_STRING,
				      "NetCDF Error (%i: %s) writing axis '%s' values to metafile, for variable %s (table: %s)",
				      ierr, nc_strerror( ierr ),
				      cmor_axes[cmor_vars[var_id].axes_ids
						[i]].id,
				      cmor_vars[var_id].id,
		...
		for( j = 0;
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF Error (%i: %s) writing axis '%s' value number %d (%s), for variable %s (table: %s)",
				  ierr, nc_strerror( ierr ),
				  cmor_axes[cmor_vars[var_id].
					    axes_ids[i]].id, j,
				  cmor_axes[cmor_vars[var_id].
...
/* -------------------------------------------------------------------- */
		...
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NC error (%i: %s) on variable %s writing bounds for dim %i (%s), for variable %s (table: %s)",
			      ierr, nc_strerror( ierr ),
			      cmor_vars[var_id].id, i,
			      cmor_axes[cmor_vars[var_id].axes_ids[i]].id,
			      cmor_vars[var_id].id,
...
/* -------------------------------------------------------------------- */
	for( i = 0; i < CMOR_MAX_DIMENSIONS; i++ ) {
		...
		if( cmor_tables[cmor_axes[j].ref_table_id].axes
		    [cmor_axes[j].ref_axis_id].type == 'c' ) {
		    ierr =
			nc_put_var_text( ncid, nc_singletons[i],
					 cmor_tables[cmor_axes
						     [j].ref_table_id].axes
					 [cmor_axes[j].
					  ref_axis_id].cvalue );
		    ...
		    ierr =
			nc_put_var_double( ncid, nc_singletons[i],
					   cmor_axes[j].values );
		}
		if( ierr != NC_NOERR ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "NetCDF Error (%i: %s) writing scalar variable %s for variable %s (table: %s), value: %lf",
			      ierr, nc_strerror( ierr ), cmor_axes[j].id,
			      cmor_vars[var_id].id,
			      cmor_tables[cmor_vars[var_id].
					  ref_table_id].table_id,
...
/* -------------------------------------------------------------------- */
		if( cmor_axes[j].bounds != NULL ) {	
		    ...
		    if( ierr != NC_NOERR ) {
			snprintf( msg, CMOR_MAX_STRING,
				  "NetCDF Error (%i: %s) writing scalar bounds variable %s for variable %s (table: %s), values: %lf, %lf",
				  ierr, nc_strerror( ierr ),
				  cmor_axes[j].id, cmor_vars[var_id].id,
				  cmor_tables[cmor_vars
					      [var_id].ref_table_id].
...
/* -------------------------------------------------------------------- */
    ...
    strncpytrim( tmp, cmor_tables[cmor_vars[var_id].ref_table_id].product,
		 CMOR_MAX_STRING );
/* -------------------------------------------------------------------- */
/*      make sure you replace spaces with "_"                           */
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
    ...
    if( cmor_check_expt_id
	( tmp, cmor_vars[var_id].ref_table_id, "experiment",
	  "experiment_id" ) != 0 ) {
...
/* -------------------------------------------------------------------- */
    ...
    for( i = 0; i <= cmor_tables[cmor_vars[var_id].ref_table_id].nexps;
	 i++ ) {
	j = strlen( tmp );
	if( strncmp
	    ( cmor_tables[cmor_vars[var_id].ref_table_id].expt_ids[i], tmp,
	      j ) == 0 ) {
	    if( strlen
		( cmor_tables[cmor_vars[var_id].ref_table_id].sht_expt_ids
		  [i] ) != 0 ) {
		strncpy( tmp2,
			 cmor_tables[cmor_vars[var_id].
...
/*      frequency                                                       */
/* -------------------------------------------------------------------- */
    if( ( cmor_tables[cmor_vars[var_id].ref_table_id].frequency[0] ==
	  '\0' )
	&&
	( cmor_tables[cmor_vars[var_id].ref_table_id].vars
	  [cmor_vars[var_id].ref_var_id].frequency[0] == '\0' ) ) {
/* -------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
	...
	if( ( cmor_tables
	    ...
	    interval =
		cmor_convert_interval_to_seconds( cmor_tables
						  [cmor_axes
						   [cmor_vars
						    [var_id].axes_ids
						    [0]].ref_table_id].
	...
	strncpy( tmp,
		 cmor_tables[cmor_vars[var_id].
			     ref_table_id].vars[cmor_vars[var_id].
						ref_var_id].frequency,
		 CMOR_MAX_STRING );
    } else {
	strncpy( tmp,
		 cmor_tables[cmor_vars[var_id].ref_table_id].frequency,
		 CMOR_MAX_STRING );
	if( strcmp( tmp, "fx" ) == 0 )
	    isfixed = 1;
    }
...
/* -------------------------------------------------------------------- */
	...
	for( i = 0;
	     i <
	     strlen( cmor_tables
		     [cmor_vars[var_id].ref_table_id].frequency ) - 3;
	     i++ ) {
	    if( strncmp
		( &cmor_tables[cmor_vars[var_id].ref_table_id].frequency
		  [i], "Clim", 4 ) == 0 ) {
...
/* -------------------------------------------------------------------- */
	...
	cmor_set_cur_dataset_attribute_internal( "physics_version", "0",
						 0 );
	cmor_set_cur_dataset_attribute_internal( "initialization_method",
						 "0", 0 );
    } else {
...
/* -------------------------------------------------------------------- */
	...
	snprintf( tmp, CMOR_MAX_STRING, "r%d",
		  cmor_current_dataset.realization );
...
/* -------------------------------------------------------------------- */
			...
			if( cmor_vars[var_id].ntimes_written_coords[i] ==
			    -1 ) {
			    sprintf( ctmp, "no" );
			} else {
			    sprintf( ctmp, "%i",
			...
			snprintf( msg, CMOR_MAX_STRING,
				  "while closing variable %i (%s, table %s) we noticed it has a time varying grid, you wrote %s time steps for the variable, but its associated variable %i (%s) has %s times written",
	...
	for( i = 0; i < 10; i++ ) {
		...
		if( cmor_vars[var_id].ntimes_written !=
		    ...
		    sprintf( ctmp, "%i",
			     ...
			     [i] );
		    snprintf( msg, CMOR_MAX_STRING,
			      "while closing variable %i (%s, table %s) we noticed it has a time varying associated variable, you wrote %s time steps for the variable, but its associated variable %i (%s) has %s times written",
...
/* -------------------------------------------------------------------- */
	...
	if( cmor_tables
		...
		snprintf( msg, CMOR_MAX_STRING,
			  "Cannot convert times for calendar: %s, closing variable %s (table: %s)",
...
/* -------------------------------------------------------------------- */
		...
		if( rename( cmor_vars[var_id].current_path, msg ) == 0 ) {
		    snprintf( msg, CMOR_MAX_STRING,
			      "Output file ( %s ) already exists, remove file or use CMOR_REPLACE or CMOR_APPEND for CMOR_NETCDF_MODE value in cmor_setup for convenience the file you were trying to write has been saved at: %s.copy",
			      outname, outname );
		} else {
		    snprintf( msg, CMOR_MAX_STRING,
			      "Output file ( %s ) already exists, remove file or use CMOR_REPLACE or CMOR_APPEND for CMOR_NETCDF_MODE value in cmor_setup.",
	...
	if( rename( cmor_vars[var_id].current_path, outname ) != 0 ) {
	    snprintf( msg, CMOR_MAX_STRING,
		      "could not rename temporary file: %s to final file name: %s",
...
/* -------------------------------------------------------------------- */
	...
	if( sz > maxsz ) {
	    sprintf( msg,
		     "Closing file: %s, size is greater than 4Gb, while this is acceptable it may be unreadable on older file systems",
		     outname );
	    cmor_handle_error( msg, CMOR_WARNING );
	}
...
int cmor_close( void ) {
	...
	if( cmor_vars[i].initialized != -1 ) {
	    ...
	    snprintf( msg, CMOR_MAX_STRING,
		      "variable %s (%i, table: %s) has been defined but never initialized",
		      cmor_vars[i].id, i,
